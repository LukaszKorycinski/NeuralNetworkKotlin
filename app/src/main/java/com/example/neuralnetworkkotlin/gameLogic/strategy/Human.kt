...
class Soldier(var look: Look) {

    var position = Vector2f(0f)
    var velocity = Vector2f(0f)
    var box = Vector2f(0.2f, 0.4f)
    var health = 10


    private fun randomPosition(): Soldier {
        position = Vector2f((Math.random() * 3).toFloat(), (Math.random() * 8).toFloat())
        return this
    }
    private fun randomLook(): Soldier {
    ...
    companion object {
        fun random(): Soldier {
            ...
            return Soldier(look).randomPosition().randomLook()
        }
    }
}
